...
const NPM_SCOPE      = "@wf-dynamic-forms",
      SRC_PATH       = "./packages",
      DIST_BASE_PATH = "./dist",
...
PACKAGES_NAMES.forEach(packageName => {
        ...
        gulp.task(TASK_NAME_DISTRIBUTE_META,
        ...
        gulp.task(TASK_NAME_DISTRIBUTE_ESM5,
            TASK_COPY([`${PACKAGE_TMP_ES5_PATH}/**/*.js`, `!${PACKAGE_TMP_ES5_PATH}/**/*.@(config.js|js.map)`], `${PACKAGE_DIST_PATH}/esm5`));

        gulp.task(TASK_NAME_DISTRIBUTE_ESM2015,
            TASK_COPY([`${PACKAGE_TMP_ES2015_PATH}/**/*.js`, `!${PACKAGE_TMP_ES2015_PATH}/**/*.@(config.js|js.map)`], `${PACKAGE_DIST_PATH}/esm2015`));
        ...
        gulp.task(TASK_NAME_DOC,
            TASK_TYPEDOC([`${PACKAGE_SRC_PATH}/src/**/!(*.spec).ts`], {
                ...
                module: "commonjs",
                name: `@wf-dynamic-forms/${packageName}`,
        ...
        gulp.task(TASK_NAME_BUILD, done => {
            runSequence(
                ...
                TASK_NAME_DISTRIBUTE_META,
                TASK_NAME_DISTRIBUTE_ESM5,
                TASK_NAME_DISTRIBUTE_ESM2015,
                TASK_NAME_BUNDLE,
                done
            );
        });
...
gulp.task("build:packages", done => {
    runSequence(...PACKAGES_NAMES.map(packageName =>
        `build:${packageName}`
    ), done)
});
